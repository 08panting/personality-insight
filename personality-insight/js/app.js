import { QUESTIONS, LIKERT_LABELS, SECTIONS, TOTAL_QUESTIONS, TRAIT_META, FACET_META, VALUE_META, RIASEC_META } from './questions.js';
import {
  scoreAssessment,
  interpretTrait,
  buildRadarPoints,
...
const STORAGE_KEY = 'personality-insight-v3';
...
function saveProgress() {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ answers, currentIndex })); } catch { /* */ }
}
...
function getSectionProgress() {
  const sectionIds = [...new Set(QUESTIONS.map((q) => q.section))];
  const current = QUESTIONS[currentIndex]?.section;
...
function renderQuestion() {
  ...
  $('#progress-fill').style.width = `${((currentIndex + 1) / TOTAL_QUESTIONS) * 100}%`;
  $('#progress-text').textContent = `${currentIndex + 1} / ${TOTAL_QUESTIONS}`;
  $('#section-label').textContent = `${section.label} (${secProg.current}/${secProg.total})`;
  $('.progress-bar').setAttribute('aria-valuenow', String(Math.round(((currentIndex + 1) / TOTAL_QUESTIONS) * 100)));
  ...
  $('#next-btn').disabled = answers[q.id] == null;
}
...
function downloadReport() {
  const text = generateReportText(scoreAssessment(answers));
  const blob = new Blob([text], { type: 'text/plain;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `真我洞察-全面报告-${new Date().toISOString().slice(0, 10)}.txt`;
  a.click();
  URL.revokeObjectURL(url);
}

$('#start-btn').addEventListener('click', () => {
  if (loadProgress() && !confirm('检测到未完成的评估进度，是否继续？\n（选择"取消"将重新开始）')) {
    clearProgress();
  }
  lastSection = currentIndex > 0 ? QUESTIONS[currentIndex - 1]?.section : null;
  showView(assessment);
  renderQuestion();
});
...
$('#next-btn').addEventListener('click', () => {
  if (answers[QUESTIONS[currentIndex].id] == null) return;
